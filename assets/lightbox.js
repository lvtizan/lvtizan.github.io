...
(function () {
  ...
  var items = imgs.map(function (img) {
    ...
    return { el: img, cap: cap, tall: !!(fig && fig.classList.contains('full-site-capture')) };
  });
  ...
  css.textContent = [
    ...
    '.lb.on{display:flex}',
    '.lb.lb-tall{align-items:flex-start;overflow:auto;padding:76px clamp(8px,2vw,24px) 32px}',
    ...
    '.lb-tall .lb-fig{width:min(1600px,96vw);max-width:96vw;align-items:stretch}.lb-tall .lb-img{width:100%;max-width:100%;max-height:none;height:auto;object-fit:initial}.lb-tall .lb-btn,.lb-tall .lb-count{position:fixed}.lb-tall .lb-nav{top:50%}',
  ...
  function show(n) {
    ...
    var it = items[i];
    ov.classList.toggle('lb-tall', it.tall);
    ov.scrollTop = 0;
